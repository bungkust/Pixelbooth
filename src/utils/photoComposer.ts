...
export async function composeResultForReview(p: any, frames: any[], template: Template): Promise<any> {
  ...
  const footerH = 40; // Smaller footer
  const headerH = (config.mainText && config.mainText.trim()) || (config.subText && config.subText.trim()) ? 60 : 0; // Smaller header
  ...
  if ((config.mainText && config.mainText.trim()) || (config.subText && config.subText.trim())) {
    ...
    if (config.mainText && config.mainText.trim()) {
      out.textSize(24); // Smaller text for review
      out.text(config.mainText.trim(), W / 2, headerY - 8);
    }
    
    if (config.subText && config.subText.trim()) {
      out.textSize(16); // Smaller text for review
      out.text(config.subText.trim(), W / 2, headerY + 12);
    }
  }
  ...
  out.textAlign(p.CENTER, p.CENTER);
  out.textFont('monospace'); 
  out.textSize(14); // Smaller text for review
